...
class Model:
    ...
    def retrieveTaskById(taskId, runTaskId, runPluginId):
        ...
        if connection and Model.__checkTasksTable(connection):
            ...
            parameters = result[-1].split('|||')

            # format result as a task object
            if result:
                return Task.taskWithId(result[0], result[1], result[2], result[3], result[4], result[5], result[6], result[7], result[8], result[9], result[10], parameters, runPluginId, runTaskId, shouldImportOnStart=result[11])
